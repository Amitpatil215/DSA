...
Discuss : https://leetcode.com/problems/find-closest-node-to-given-two-nodes/discuss/2357744/BFS-oror-Concept-of-Shortest-path-of-a-node-from-multiple-sources
...
class Solution {
   public:
    vector<int> shortestDistanceBFS(vector<int>& edges, int src) {
        queue<int> q;
        ...
        while (!q.empty()) {
            int front = q.front();
            q.pop();
            if (edges[front] != -1 && dist[edges[front]] > dist[front] + 1) {
                dist[edges[front]] = dist[front] + 1;
                q.push(edges[front]);
            }
        }
        return dist;
    ...
    int closestMeetingNode(vector<int>& edges, int node1, int node2) {
        auto dist1 = shortestDistanceBFS(edges, node1);
        auto dist2 = shortestDistanceBFS(edges, node2);

        int minDist = INT_MAX;
        int node = -1;
        for (int i = 0; i < dist1.size(); i++) {
            if (minDist > max(dist1[i], dist2[i]) && dist1[i] != (1e9) &&
                dist2[i] != (1e9)) {
