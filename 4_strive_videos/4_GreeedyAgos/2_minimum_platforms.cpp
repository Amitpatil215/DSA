...
function findPlatform() which takes the array arr[] (denoting the arrival
times), array dep[] (denoting the departure times) and the size of the array as
inputs and returns the minimum number of platforms required at the railway
...
class Solution{
    ...
    // sort both arrival and depature time
    // if arrival time of next train is more than depature time of prev ->
    // reduce the platform count & move to next index in depature
    // otherwise increase the platofrm count & move to next index in arrival
    ...
    int findPlatform(int arr[], int dep[], int n)
    {
        ...
        int index_de=0;
    	while(index_ar<n-1 ){
            if(dep[index_de] >= arr[index_ar+1]){
                currCount++;
                index_ar++;
            }else{
                currCount--;
            ...
            maxPlatforms = max(maxPlatforms,currCount);
