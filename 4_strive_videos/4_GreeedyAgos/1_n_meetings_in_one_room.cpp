...
class Solution {
   public:
    ...
    // sort according to the end time of the meetings
    // if next time is greater than current end time then we can conduct a
    // meeting
    int maxMeetings(int start[], int end[], int n) {
        ...
        pair<int, int> earlier = meeting[0];
        int count = 1;
        for (int i = 1; i < n; i++) {
            if (earlier.first < meeting[i].second) {
                count++;
                earlier = meeting[i];
            }
        }
